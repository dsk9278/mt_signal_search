...
class SQLiteSignalRepository(SignalRepository):
    ...
    def add_logic_equation(
        self, target_signal_id: str, raw_expr: str, source_label: str = "", source_page: int = None
    ) -> None:
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute(
    ...
    def add_box_connection(self, conn_rec: BoxConnection) -> None:
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    conn_rec.from_box_name,
                    conn_rec.from_box_no,
                    conn_rec.kabel_no,
                    conn_rec.to_box_no,
                    conn_rec.to_box_name,
                ),
            )
            conn.commit()
    ...
    def get_logic_expr(self, target_signal_id: str) -> str | None:
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute(
                "SELECT raw_expr FROM logic_equations WHERE target_signal_id =  ?",
                (target_signal_id,),
            )
            row = c.fetchone()
    ...
    def get_source_label(self, target_signal_id: str) -> str | None:
        with sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute(
                "SELECT source_label FROM logic_equations WHERE target_signal_id = ?",
                (target_signal_id,),
            )
            row = c.fetchone()
            return row[0] if row and row[0] else None
