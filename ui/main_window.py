...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self) -> None:
        ...
        content_l.setContentsMargins(40, 0, 40, 40)
    ...
    def _add_signal_via_gui(self) -> None:
        ...
        from_edit = QLineEdit()
        via_edit = QLineEdit()
        via_edit.setPlaceholderText("カンマ区切り（例: BOX5,BOX6）")
        to_edit = QLineEdit()
        addr_edit = QLineEdit()
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            if not sid or not desc:
                QMessageBox.warning(self, "入力不備", "信号IDと説明は必須です。")
                return
    ...
    def _open_edit_signal_dialog(self):
        ...
        info = SignalInfo(
            ...
            via,
            data.get("to_box"),
            data.get("program_address") or data.get("signal_id"),
    ...
    def _import_csv_box(self):
        ...
        self._connect_common_worker_signals(
            worker,
            thread,
            title="CSV取り込み中（BOX配線）",
            label="box_connections.csv を読み込んでいます…",
        )
        ...
        try:
            ...
            if not log_path.exists():
                QMessageBox.information(
                    self,
                    "ログ",
                    r"まだログファイルが作成されていません。\(n(CSV/PDFの取り込みを一度実行すると作成されます。)",
                )
                return
